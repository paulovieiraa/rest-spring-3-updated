...
public class PersonServiceImpl implements PersonService {
  ...
  public PersonModel create(PersonInput input) {
    ...
    personModel.add(
        linkTo(methodOn(PersonController.class).getPerson(p.getId().toString()))
            .withRel("getById"));
    personModel.add(
        linkTo(methodOn(PersonController.class).deletePerson(p.getId().toString()))
            .withRel("deleteById"));
    return personModel;
  }
  ...
  public Page<PersonModel> getPeople(Pageable pageable) {
    ...
    return people.map(
        person -> {
          ...
          personModel.setCpf(
              "***."
                  + person.getCpf().substring(3, 6)
                  + ".***-"
                  + person.getCpf().substring(9, 11));
          return personModel;
        });
  }
  ...
  public PersonModel getPerson(String id) {
    ...
    personModel.add(
        linkTo(methodOn(PersonController.class).deletePerson(id))
            .withRel("delete")
  ...
  public List<PersonModel> getPersonByName(String name) {
    List<Person> personList = personRepository.findByFirstNameIgnoreCaseAndAccent(name);
    return mapper.convertValue(
        personList, mapper.getTypeFactory().constructCollectionType(List.class, PersonModel.class));
  }
